...
class CommunionController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $canParishView = (bool) ($user?->can('communions.parish.view'));
        $canLeaderView = (bool) ($user?->can('communions.view'));
        ...
        return Inertia::render('Sacraments/Communions/Index', [
            'filters' => [
                'q' => $q,
    ...
    public function create(Request $request): Response
    {
        ...
        return Inertia::render('Sacraments/Communions/Create', [
            'cycles' => $cycles,
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if (! $user || ! $user->can('communions.register')) {
            abort(403);
        }
        ...
        $isTransfer = (bool) ($validated['is_transfer'] ?? false);
        ...
        try {
            ...
            if ($existing) {
                return redirect()->route('communions.show', $existing)->with('info', 'A registration already exists for this candidate in the selected cycle.');
            }

            throw $e;
        }

        return redirect()->route('communions.show', $reg)->with('success', 'Communion registration created.');
    }
    ...
    public function show(Request $request, SacramentProgramRegistration $registration): Response
    {
        ...
        return Inertia::render('Sacraments/Communions/Show', [
    ...
    public function submit(Request $request, SacramentProgramRegistration $registration): RedirectResponse
    {
        ...
        $hasBaptismCert = DB::table('sacrament_attachments')
            ->where('entity_type', 'program_registration')
            ->where('entity_id', (int) $registration->id)
            ->where('type', SacramentProgramRegistration::TYPE_BAPTISM_CERTIFICATE)
            ->exists();

        if (! $hasBaptismCert) {
            return back()->with('error', 'Baptism certificate is required before submitting.');
        }

        if ((bool) $registration->is_transfer) {
            $hasParishLetter = DB::table('sacrament_attachments')
                ->where('entity_type', 'program_registration')
                ->where('entity_id', (int) $registration->id)
                ->where('type', SacramentProgramRegistration::TYPE_PARISH_LETTER_COMMUNION_STUDY)
                ->exists();

            if (! $hasParishLetter) {
                return back()->with('error', 'Parish letter is required for transfer candidates before submitting.');
            }
        }
    ...
    public function approve(Request $request, SacramentProgramRegistration $registration): RedirectResponse
    {
        ...
        $user = $request->user();
        if (! $user || ! $user->can('communions.approve')) {
            abort(403);
        }
    ...
    public function reject(Request $request, SacramentProgramRegistration $registration): RedirectResponse
    {
        ...
        $user = $request->user();
        if (! $user || ! $user->can('communions.reject')) {
            abort(403);
        }
    ...
    public function complete(Request $request, SacramentProgramRegistration $registration): RedirectResponse
    {
        ...
        $user = $request->user();
        if (! $user || ! $user->can('communions.complete')) {
            abort(403);
        }
    ...
    public function issue(Request $request, SacramentProgramRegistration $registration): RedirectResponse
    {
        ...
        $user = $request->user();
        if (! $user || ! $user->can('communions.issue')) {
            abort(403);
        }
