...
export default function SearchableJumuiyaSelect({
    id,
    label,
    value,
    onChange,
    ...
    const fetchOptions = async (nextQuery) => {
        ...
        try {
            const res = await axios.get(route('jumuiyas.lookup'), {
                params: {
                ...
                {() => (
                    <div className="relative">
                        <Combobox.Input
                            ...
                            placeholder="Search Christian Community..."
                            onFocus={() => {
                                setOpen(true);
                        ...
                        <Combobox.Button
                            ...
                            aria-label="Toggle Christian communities"
                            onClick={() => setOpen((v) => !v)}
                        >
                            {loading ? (
                                ...
                                {options.length === 0 ? (
                                    <div className="px-3 py-2 text-slate-500">
                                        {disabled
                                            ...
                                            : query.trim() !== ''
                                                ? 'No matches.'
                                                : 'Click the arrow to view Christian communities, or type to search.'}
                                    </div>
                                ) : (
                                    options.map((opt) => (
                                        <Combobox.Option
