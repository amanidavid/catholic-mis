...
export default function SearchableInstitutionSelect({
    id,
    label,
    value,
    onChange,
    ...
    const fetchOptions = async (nextQuery) => {
        ...
        try {
            const res = await axios.get(route('institutions.lookup'), {
                params: {
                            ...
                            >
                                {options.length === 0 ? (
                                    <div className="px-3 py-2 text-slate-500">
                                        {query.trim() !== '' ? 'No matches.' : 'Click the arrow to view institutions, or type to search.'}
                                    </div>
                                ) : (
                                    options.map((opt) => (
                                        <Combobox.Option
                                            key={opt.uuid}
                                            value={opt}
                                            className={({ active }) => `cursor-pointer px-3 py-2 ${active ? 'bg-indigo-50 text-indigo-900' : 'text-slate-700'}`}
                                        >
                                            <div className="font-semibold">{opt.name}</div>
                                            {!!opt.type && <div className="text-xs text-slate-500">{opt.type}</div>}
                                        </Combobox.Option>
                                    ))
                                )}
                            </Combobox.Options>
