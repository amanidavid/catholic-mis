...
export default function SearchableFamilySelect({
    id,
    label,
    value,
    onChange,
    ...
    const fetchOptions = async (nextQuery) => {
        ...
        try {
            const res = await axios.get(route('families.lookup'), {
                params: {
                            ...
                            >
                                {options.length === 0 ? (
                                    <div className="px-3 py-2 text-slate-500">
                                        {disabled ? 'Select a Christian Community first.' : query.trim() !== '' ? 'No matches.' : 'Click the arrow to view families, or type to search.'}
                                    </div>
                                ) : (
                                    options.map((opt) => (
                                        <Combobox.Option
                                        ...
                                        >
                                            <div className="flex items-center justify-between gap-3">
                                                ...
                                                <div className="text-xs text-slate-500">{opt.code ?? ''}</div>
                                            </div>
                                        </Combobox.Option>
                                    ))
                                )}
