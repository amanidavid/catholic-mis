...
export default function Families({ scoped_jumuiya, can_select_jumuiya }) {
    ...
    const run = async (nextPage = 1) => {
        ...
        setLoading(true);
        try {
            const res = await axios.get(route('weekly-attendance.reports.families.data'), {
                params: {
                    from,
                    to,
    ...
    const exportExcel = () => {
        ...
        const url = route('weekly-attendance.reports.families.export', {
            from,
            to,
            jumuiya_uuid: jumuiyaUuid,
        });
    ...
    if (!canView) {
        return (
            <AuthenticatedLayout>
                <Head title="Family Attendance Report" />
                <div className="mx-auto max-w-5xl">
                    <div className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-slate-200/70">
                        <h1 className="text-xl font-semibold text-slate-900">Family Attendance Report</h1>
            ...
            <div className="mx-auto max-w-6xl space-y-6">
                <div className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-slate-200/70">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <h1 className="text-xl font-semibold text-slate-900">Family Attendance Report</h1>
                            <p className="mt-1 text-sm text-slate-500">Aggregated weekly attendance counts per family. Default range is current month.</p>
                        </div>
