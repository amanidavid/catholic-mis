...
export default function ConfirmationsIndex({ registrations, filters, cycles }) {
    ...
    const submitSearch = (e) => {
        e.preventDefault();
        router.get(route('confirmations.index'), { q, cycle_uuid: cycleUuid, from, to }, { preserveState: true, preserveScroll: true, replace: true });
    };
    ...
    const clearSearch = () => {
        ...
        setFrom('');
        setTo('');
        router.get(route('confirmations.index'), {}, { preserveState: true, preserveScroll: true, replace: true });
    };
        ...
        <AuthenticatedLayout>
            ...
            <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
                <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Confirmation Registrations</h1>
                        <p className="mt-1 text-sm text-slate-600">Candidates must have completed First Communion before registering for Confirmation.</p>
                    </div>
                    {can('confirmations.register') && (
                        <PrimaryButton
                            type="button"
                            className="h-11 bg-indigo-700 hover:bg-indigo-800"
                            onClick={() => router.get(route('confirmations.create'))}
                        >
                                    ...
                                    rows.map((r) => {
                                        ...
                                        return (
                                            <tr
                                                ...
                                                onClick={() => router.get(route('confirmations.show', r.uuid))}
                                            >
                                                ...
                                                <td className="whitespace-nowrap text-right">
                                                    <button
                                                        ...
                                                        onClick={(e) => {
                                                            e.preventDefault();
                                                            e.stopPropagation();
                                                            router.get(route('confirmations.show', r.uuid));
                                                        }}
                                                    >
                                                        {openLabel}
                                                    </button>
