...
export default function CommunionsShow({ registration, cycles }) {
    ...
    const canEditDraft = can('communions.register') && ['draft', 'rejected'].includes((r?.status ?? '').toString().toLowerCase());
    const canSubmit = can('communions.register') && ['draft', 'rejected'].includes((r?.status ?? '').toString().toLowerCase());

    const hasAnyAttachment = (Array.isArray(r?.attachments) ? r.attachments : []).length > 0;

    const canApprove = can('communions.approve') && (r?.status ?? '') === 'submitted';
    const canReject = can('communions.reject') && (r?.status ?? '') === 'submitted';
    const canComplete = can('communions.complete') && (r?.status ?? '') === 'approved';
    const canIssue = can('communions.issue') && (r?.status ?? '') === 'completed';

    const missingRequirements = useMemo(() => {
        const missing = [];
        const baptismCert = (attachmentsByType?.baptism_certificate ?? []).length > 0;
        if (!baptismCert) missing.push('Baptism certificate');

        if (r?.is_transfer) {
            const letter = (attachmentsByType?.parish_letter_communion_study ?? []).length > 0;
            if (!letter) missing.push('Parish letter');
        }

        return missing;
    }, [attachmentsByType, r?.is_transfer]);
    ...
    const saveDraft = (e) => {
        e.preventDefault();
        draftForm.post(route('communions.draft.save', r.uuid), {
            preserveScroll: true,
    ...
    const submitRegistration = () => {
        router.post(route('communions.submit', r.uuid), {}, { preserveScroll: true, onSuccess: () => router.reload({ only: ['registration'] }) });
    };

    const approve = () => {
        router.post(route('communions.approve', r.uuid), {}, { preserveScroll: true, onSuccess: () => router.reload({ only: ['registration'] }) });
    };

    const reject = (e) => {
        e.preventDefault();
        rejectForm.post(route('communions.reject', r.uuid), {
            preserveScroll: true,
            onSuccess: () => {
                setRejectOpen(false);
    ...
    const complete = () => {
        router.post(route('communions.complete', r.uuid), {}, { preserveScroll: true, onSuccess: () => router.reload({ only: ['registration'] }) });
    };

    const issue = () => {
        router.post(route('communions.issue', r.uuid), {}, { preserveScroll: true, onSuccess: () => router.reload({ only: ['registration'] }) });
    };
    ...
    return (
        <AuthenticatedLayout>
            <Head title="Communion Registration" />

            <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
                <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <h1 className="text-2xl font-semibold tracking-tight text-slate-900">First Communion Registration</h1>
                ...
                <div className="mb-6 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            {r?.is_transfer ? (
                                <span className="inline-flex items-center rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-800 ring-1 ring-amber-200">Transfer</span>
                            ) : null}
                        </div>
                        ...
                        <form onSubmit={saveDraft} className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
                            ...
                            <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
                                <label className="inline-flex items-center gap-2 text-sm font-semibold text-slate-800">
                                    <input
                                        type="checkbox"
                                        checked={!!draftForm.data.is_transfer}
                                        onChange={(e) => draftForm.setData('is_transfer', e.target.checked)}
                    ...
                    <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
                        <div className="text-xs font-semibold uppercase tracking-wide text-slate-600">Submit readiness</div>
                        {missingRequirements.length === 0 ? (
                            <div className="mt-2 text-sm text-emerald-800">Ready to submit.</div>
                        ) : (
                            <div className="mt-2 text-sm text-amber-900">Missing: <span className="text-xs">{missingRequirements.join(', ')}</span></div>
                        )}
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
                    <div className="lg:col-span-3 space-y-6">
                        <AttachmentList
                            ...
                            required
                            hint="PDF only, max 3MB. Required before submission."
                        />

                        {r?.is_transfer ? (
                            <AttachmentList
                                type="parish_letter_communion_study"
                                title="Parish letter (communion study)"
                                required
                                hint="Required for transfer candidates before submission."
                            />
                        ) : null}
                    </div>
                            ...
                            <form onSubmit={upload} className="mt-4 space-y-3">
                                <div>
                                    ...
                                    >
                                        ...
                                        <option value="baptism_certificate">Baptism certificate (required)</option>
                                        <option value="parish_letter_communion_study">Parish letter (transfer case)</option>
                                    </FloatingSelect>
                                </div>
