...
export default function CommunionCyclesIndex({ cycles }) {
    const { auth } = usePage().props;
    const permissions = auth?.user?.permissions ?? [];
    const canManage = Array.isArray(permissions) && permissions.includes('communions.cycles.manage');
    ...
    const setStatus = (cycleUuid, status) => {
        router.post(route('communions.cycles.status', cycleUuid), { status }, { preserveScroll: true });
    };

    return (
        <AuthenticatedLayout>
            <Head title="Communion Cycles" />

            <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
                <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Communion Cycles</h1>
                        <p className="mt-1 text-sm text-slate-600">Control registration windows for First Communion.</p>
                    </div>
                    {canManage ? (
                        <PrimaryButton type="button" className="h-11 bg-indigo-700 hover:bg-indigo-800" onClick={() => router.get(route('communions.cycles.create'))}>
                                            ...
                                            <tr key={c.uuid} className={`${idx % 2 === 1 ? 'bg-slate-50/50' : 'bg-white'} hover:bg-indigo-50/40 transition`}>
                                                ...
                                                <td className="whitespace-nowrap px-4 py-3 text-right">
                                                    <div className="flex flex-wrap items-center justify-end gap-2">
                                                        <button
                                                            type="button"
                                                            onClick={() => router.get(route('communions.cycles.edit', c.uuid))}
