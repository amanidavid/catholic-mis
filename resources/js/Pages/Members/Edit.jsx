...
export default function MembersEdit({ member }) {
    ...
    const {
        ...
        family_relationship_uuid: m?.family_relationship_uuid ?? '',
    ...
    const canSubmit = useMemo(() => {
        return !!data.family_uuid && !!data.first_name && !!data.last_name;
    }, [data]);

    const submit = (e) => {
        e.preventDefault();
        patch(route('members.update', m.uuid), { preserveScroll: true });
    };
            ...
            <div className="mx-auto max-w-4xl space-y-6">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">Update member details.</p>
                    </div>
                        ...
                        <div className="mt-3 grid gap-4 md:grid-cols-2">
                            ...
                            {isScoped && (
                                <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3">
                                    ...
                                    <div className="mt-1 text-sm font-semibold text-slate-900 break-words">{m?.zone_name || '-'}</div>
                                </div>
                            )}

                            {!isScoped && (
                                <SearchableJumuiyaSelect
                                    id="edit_jumuiya_uuid"
                            ...
                            {isScoped && (
                                <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3">
                                    ...
                                    <div className="mt-1 text-sm font-semibold text-slate-900 break-words">{m?.jumuiya_name || '-'}</div>
                                </div>
                            )}
